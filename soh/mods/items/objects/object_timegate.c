...
void CustomItems_DrawTimeGatePortal(Player* player, PlayState* play) {
    if (!tgPortalActive || tgPortalAlpha <= 0.0f) return;

    OPEN_DISPS(play->state.gfxCtx);

    // Update scroll animation
    sPortalScrollOffset += 15.0f;
    if (sPortalScrollOffset > 512.0f) sPortalScrollOffset -= 512.0f;
    ...
    gSPSegment(POLY_XLU_DISP++, 0x08,
               Gfx_TwoTexScroll(play->state.gfxCtx,
                   0, scrollTime & 0xFF, -((s16)(scrollTime * 2) & 511), 0x100, 0x100,
    ...
    if (tgPortalAlpha > 128.0f) {
        ...
        if (rayAlpha > 255.0f) rayAlpha = 255.0f;
        ...
        gSPSegment(POLY_XLU_DISP++, 0x08,
                   Gfx_TwoTexScroll(play->state.gfxCtx,
                       0, scrollTime2 & 0xFF, -((s16)scrollTime & 511), 0x100, 0x100,
